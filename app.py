...
async def post_detail(request: Request, post_id: str):
    ...
    with engine.connect() as conn:
        ...
        with engine.connect() as conn:
            result = conn.execute(text(" SELECT commnets.content users.username AS user_name FROM comments JOIN users ON comments.user_id = users.id WHERE comments.post_id - :post_id ORDER BY comments.created_at DESC"), {"post_id": post_id})
            rows = result.fetchall()
