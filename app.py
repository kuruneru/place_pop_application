...
async def first(request: Request):
    with engine.connect() as conn:
        result = conn.execute(text("SELECT posts.id, posts.title, posts.place_name, posts.address, posts.image_filename, posts.poster_type, posts.location_type, posts.created_at, posts.updated_at, posts.annotation, users.username AS user_name, post_evaluations.id AS evaluation_id FROM posts JOIN users ON posts.user_id = users.id LEFT JOIN post_evaluation ON post_evaluation.post_id = posts.id ORDER BY posts.created_at DESC"))
...
async def post_detail(request: Request, post_id: str):
    ...
    return templates.TemplateResponse("post_detial.html",{"request": request, "post": dict(post._mapping), "comments": comments})
...
async def like_tourist_post(request: Request, post_id: str):
    ...
    with engine.connect() as conn:
        ...
        conn.commit()
    return RedirectResponse(url=f"/", status_code_=303)
    # return RedirectResponse(url=f"/posts/{post_id}", status_code=303)